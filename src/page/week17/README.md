...
Interview Mode는 한 번에 모든 결정을 끝내려 하기보다, 질문→확인→수정 루프를 통해 점진적으로 정교해지는 방식이다.

```bash
# 큰 작업은 인터뷰 방식으로 단계화
claude "현재 로그인 모듈의 오류 처리 품질을 높이고 싶어"

claude --interview "다음 순서로 점검해줘.
1) 실패 조건 정리
2) 원인 후보 추정
3) 안전한 수정안 제시
4) 검증 체크리스트 작성"
...
### Claude Code Teams (Teams 사용/미사용 비교)

동일한 요청을 Teams 기능 기준으로 나눠서 보면 결과 형태가 분명히 달라집니다.

#### Teams 사용 vs 미사용

| 항목 | Teams 사용 | Teams 미사용 |
|---|---|---|
| 출력 형태 | 산출물 중심 흐름(공유된 맥락/스냅샷) | 텍스트 정리 중심 흐름 |
| 장점 | 협업 멤버 간 의사결정 속도↑, 맥락 공유 용이 | 자동화 과제에서 결과 예측성↑, diff 리뷰 용이 |
| 단점 | 정보량이 커질 수 있음 | 디테일한 시각 맥락은 수동 보완 필요 |
| 적합한 작업 | 화면 플로우 설계, 리뷰 브리핑 | 코드 리팩토링, 대량 스크립트 변경 |
...
### Sub-Agent 병렬 처리 vs Teams 다중 의사결정 비교

같은 과업이라도 처리 방식에 따라 결과 품질과 협업 비용이 달라지므로, 먼저 이 부분을 구분해서 시작한다.

#### 사용 시점 가이드

| 구분 | Sub-Agent 병렬 | Teams 다중 의사결정 |
|---|---|---|
| 핵심 목표 | 독립 작업을 동시에 분해해 빠르게 정리 | 요구사항 변화가 큰 문제를 협의하며 수렴 |
| 적합한 과업 | 테스트/분석/성능 측정처럼 실행 트랙이 분명한 작업 | 화면/UX/우선순위 판단처럼 판단 포인트가 많은 작업 |
| 토큰 사용 성향 | 산출물이 예측 가능해 통제가 쉬움 | 공유 근거가 많아 컨텍스트 증가 가능 |
| 의사결정 밀도 | 낮음/중간: 결과를 합치기 중심 | 높음: 여러 단계 확인과 조율 반복 |

#### 간단한 비교 프롬프트

```bash
# 동일 과업을 두 방식으로 비교해볼 때
claude "로그인 실패 회복 전략을 3개 제안해줘
1) Sub-Agent 병렬 방식으로 제안
2) Teams 다중 의사결정 방식으로 제안"
```
...
AI 작업에서 가장 큰 비용은 매일 쓰는 코딩 능력 자체가 아니라, **불필요한 컨텍스트를 얼마나 줄이느냐**에서 발생한다.

## 1) Playwright MCP에서 Playwright CLI로 전환

Playwright를 MCP로 호출하면 설정은 편하지만, 반복 검증 시에는 CLI 중심으로 분기할 때가 많다.
...
```bash
# 권장: Playwright CLI + 테스트 코드
bun add -D @playwright/test
bunx playwright install

# 실패/성공이 명확한 테스트를 점진적으로 실행
bunx playwright test e2e/login.spec.ts --reporter=dot
```

효과

- 시나리오를 코드로 고정해 불필요한 설명을 줄임
- 스크립트 재사용으로 동일 컨텍스트를 반복 생성하지 않음
- 로그/스크린샷은 실패 시에만 상세화

## 2) MCP vs CDP(Chrome DevTools Protocol) 비교

작업 성격에 따라 MCP와 CDP를 섞어 쓰면 토큰 비용과 정밀도를 모두 관리할 수 있다.

| 항목 | MCP | CDP |
|---|---|---|
| 제어 수준 | 고수준 도구 호출 | 브라우저 내부 이벤트 직접 제어 |
| 컨텍스트 비용 | 초기 오케스트레이션 설명이 필요 | 스크립트 중심으로 비용 통제 용이 |
| 장점 | 생산성/간편성 | 정밀 제어/디버깅 |
| 단점 | 요청량이 크면 비용 증가 | 설정/버전 이슈 관리 필요 |

### 컨텍스트 최적화 체크리스트

- 목표·범위를 한 번의 요청에 한 문장으로 제한
- 실패 로그는 요약만 먼저 받고 상세는 필요한 시점에만
- 스크린샷/trace는 실패 케이스에서만
- 동일 요구사항은 재실행 가능한 스크립트로 모듈화
- 에이전트에게 “무엇을 하지 말지”를 함께 전달
